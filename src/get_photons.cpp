        ...
        for(;;)
        {
            ...
            switch ((int) block_header.type)
            {
                ...
                {    
                    ...
                    int jarray;
                    double photons;
                    int rc = begin_read_tel_array(iobuf, &item_header, &iarray);
                    if( rc < 0)
                    {
                        perror("Error in IO_TYPE_MC_TELARRAY");
                    }
                /*
                    while ((type = next_subitem_type(iobuf)) > 0 )
                    {
                        ...
                        {
                            rc = read_tel_photons(iobuf, max_bunches, &jarray, &itel, &photons, bunches, &nbunches);
                            if(rc < 0)
                            {
                                perror("Error when reading tel_photons");
                            }
                ...
                */
                ...
                {   
                    ...
                    double  thickstep;
                    int nx = 1071;
                    double plongi[9][1071];
                    double xlongi[1071], elongi[1071];
                    if (type == 1)
                    {
                        read_shower_longitudinal(iobuf, &ev, &type, &plongi[0][0], ndim, &np, &nthick, &thickstep, np_max);
                    }
                    for(int j = 0; j < nthick && j < nx; j++)
                    {
                        xlongi[j] = plongi[6][j];
                        elongi[j] = plongi[1][j] + plongi[2][j];
                        clongi[j] = plongi[8][j];
                    }
                }
                break;
